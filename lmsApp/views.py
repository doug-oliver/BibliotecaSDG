...
def context_data(request):
    ...
    context = {
        ...
        'page_name' : '',
        'page_title' : '',
        'system_name' : 'Biblioteca Carlo Acutis',
        'topbar' : True,
        'footer' : True,
    }
...
def users(request):
    ...
    context['users'] = User.objects.exclude(is_superuser=True).all()
...
def books(request):
    context = context_data(request)
    context['page'] = 'book'
    context['page_title'] = "Lista de Livros"
...
def students(request):
    ...
    context['page_title'] = "Lista de Seminaristas"
...
def borrows(request):
    context = context_data(request)
    context['page'] = 'borrow'
    context['page_title'] = "Empréstimos"
...
def save_borrow(request):
    ...
    if request.method == 'POST':
        ...
        if not post['id'] == '':
            ...
            # Atribuindo o usuário atual ao campo student
            form_data = request.POST.copy()
            form_data['student'] = current_user.id
            form = forms.SaveBorrow(form_data)

        if form.is_valid():
            borrow = form.save(commit=False)  # Cria a instância, mas não salva no banco ainda
            borrow.student = current_user  # Atribui o objeto User ao campo student
            borrow.save()  # Salva o objeto Borrow no banco de dados
            if post['id'] == '':
